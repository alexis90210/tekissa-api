...
if(!isset($_POST['id_categorie'])) {
    ...
    $controller->response('error', 'id_categorie non transmise');
}
...
$filename = '/public/image/'.strtolower(explode('.', $name)[0]). '-' . uniqid() . '.' . $extension;
...
if ($bool && compress_image($target, $target, 70)) {   
    ...
    try {
        ...
        $query = "UPDATE categories SET image = ? WHERE id_categorie = ?";

        $request = $db->prepare($query);

        $request->execute([ $link, intval($_POST['id_categorie']) ]);
